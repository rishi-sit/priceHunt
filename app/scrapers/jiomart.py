...
class JioMartScraper(BaseScraper):
    ...
    async def search(self, query: str) -> List[ProductResult]:
        """Search for products on JioMart (smart-buys)."""
        results = []
        
        # JioMart search URL for smart-buys
        search_url = f"{self.BASE_URL}/search/{query.replace(' ', '%20')}?tab=smart-buys"
        
        print(f"JioMart: Searching with {search_url}")
        ...
        try:
            async with await self.get_client() as client:
                ...
                if response.status_code == 200:
                    ...
                    print(f"JioMart: Got status {response.status_code}")
    ...
    def _extract_next_data(self, soup) -> Optional[dict]:
        ...
        try:
            ...
            print(f"JioMart: Next data extraction error: {e}")
        return None
    ...
    def _parse_next_data(self, data: dict) -> List[ProductResult]:
        ...
        try:
            ...
            print(f"JioMart: JSON parse error: {e}")
    ...
    def _parse_html_products(self, soup) -> List[ProductResult]:
        ...
        try:
            ...
            print(f"JioMart: HTML parse error: {e}")
